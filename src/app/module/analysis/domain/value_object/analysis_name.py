...
class AnalysisName(BaseValueObject[str]):
    """Value object for an analysis name.
    ...
    def _validate(self) -> None:
        if not (self._MIN_LENGTH <= len(self._value) <= self._MAX_LENGTH):
            message = (
                f"Analysis name must be between {self._MIN_LENGTH} and {self._MAX_LENGTH} "
        ...
        if not self._PATTERN.match(self._value):
            ...
            message = f"Analysis name must start with a letter and contain only {allowed} characters."
            raise ValidationError(message)


__all__ = ("AnalysisName",)

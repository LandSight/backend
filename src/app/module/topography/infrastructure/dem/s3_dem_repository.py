...
class S3LocalDemRepository(S3GeoRepository, LocalDemRepository):
    ...
    Inherits from :class:`S3GeoRepository` for S3/rasterio session management
    and from :class:`LocalDemRepository` for the domain port.
    """

    _TILE_PRECISION: int = 2
    _METERS_PER_DEGREE: float = 111320.0
    ...
    def __init__(
        ...
        """Initialize the S3 DEM repository.
        ...
        s3_config : S3Config
            S3-compatible storage configuration with dem_bucket.
        """
        S3GeoRepository.__init__(self, aws_session, s3_boto_client, s3_config)
        self._bucket = s3_config.dem_bucket
        self._logger = logger
    ...
    async def exists(self, bounds: BoundingBox) -> bool:
        ...
        key = self._build_key(bounds)
        try:
            self._s3_boto_client.head_object(Bucket=self._bucket, Key=key)
        except self._s3_boto_client.exceptions.ClientError:
            return False
        else:
            return True
    ...
    def _calculate_resolution_in_meters(
        ...
        # Convert degrees to meters (1 degree ≈ 111320 * cos(latitude) meters)
        resolution_meters = avg_res_deg * self._METERS_PER_DEGREE * math.cos(lat_rad)

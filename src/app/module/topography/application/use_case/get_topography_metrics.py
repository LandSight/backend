...
class GetTopographyMetricsUseCase(BaseUseCase[GetTopographyMetricsCommand, TopographyMetricsResponse]):
    ...
    async def __call__(self, command: GetTopographyMetricsCommand) -> TopographyMetricsResponse:
        self._logger.info("Getting topography metrics: parcel_id=%s", command.parcel_id)

        parcel_id = ParcelId(command.parcel_id)
        metrics = await self._metrics_repository.get_by_parcel_id(parcel_id)
        ...
        self._logger.info("Topography metrics found: id=%s parcel_id=%s", metrics.id, command.parcel_id)

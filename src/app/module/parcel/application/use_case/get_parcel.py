...
class GetParcelUseCase(BaseUseCase[GetParcelCommand, ParcelResponse]):
    ...
    Access is granted only when the current user is allowed to view it.
    """
    ...
    async def __call__(self, command: GetParcelCommand) -> ParcelResponse:
        ...
        if not await self._parcel_permission_service.user_can_view_parcel(owner_id, parcel_id):
            self._logger.warning(
                "User %s attempted to view parcel %s but is not allowed",
